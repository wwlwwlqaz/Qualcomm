...
class test_suit_cmcc_devci_browser_case2(TestCaseBase):
    '''
            ...
            if wait_for_fun(lambda:search_view_by_id('next_button'), True, 5):
                ...
                if wait_for_fun(lambda:search_view_by_id('next_button'), True, 5):
                    ...
                    if wait_for_fun(lambda:search_view_by_id('next_button'), True, 5):
                        ...
                        if wait_for_fun(lambda:search_view_by_id('next_button'), True, 5):
                            click_button_by_id('next_button')       
            if wait_for_fun(lambda:search_view_by_id("menu_button"), True, 5) or search_view_by_id("more_browser_settings"):
                log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Enter browser successfully")
                send_key(KEY_BACK)
                if wait_for_fun(lambda:search_text("QUIT"), True, 5):
                    click_button_by_text("QUIT")	                         
                    sleep(2)
                send_key(KEY_BACK)
                send_key(KEY_BACK)
                send_key(KEY_HOME)
