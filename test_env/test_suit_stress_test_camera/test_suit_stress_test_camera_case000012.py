...
class test_suit_stress_test_camera_case000012(TestCaseBase):
    '''
    ...
    '''
    "Procedure:
    Step1：launch Camera  and use front Camera;
    Step2: Switch  to Video Record for some time;
    Step3: open flashlight;
    Step4: close flashlight.
    Step5： switch between open/close for 100 time;
    ...
    def test_case_main(self, case_results):
        ...
        if wait_for_fun(lambda:search_view_by_id('scene_mode_switcher'), True, 5):
            ...
            if wait_for_fun(lambda:search_view_by_id('setting_button'), True, 8):
...
#                         click_textview_by_text("SD Card")                 
        ...
        if wait_for_fun(lambda:search_view_by_id('front_back_switcher'), True, 5):
            ...
            log_test_framework("step1:", "Launch camera and use front camera pass")
            sleep(5)        
        if wait_for_fun(lambda:search_view_by_id('video_button'), True, 5):
            click_imageview_by_id('video_button')
            sleep(3)
            log_test_framework("step2:", "Switch  to Video Record for some time pass")
            for i in range(iterationNum):
                ...
                if wait_for_fun(lambda:search_view_by_id('flash_button'), True, 5):
                    ...
                    log_test_framework("step3:", "open flashlight pass")
                    on_flag = True
                    sleep(3)
                    if wait_for_fun(lambda:search_view_by_id("flash_button"), True, 5):
                        click_imageview_by_id("flash_button")
                        log_test_framework("step4:", "close flashlight pass")
                        off_flag = True
                        sleep(3)
