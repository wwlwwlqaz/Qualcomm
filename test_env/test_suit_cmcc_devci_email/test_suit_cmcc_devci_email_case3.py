...
class test_suit_cmcc_devci_email_case3(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text('Inbox'), True, 5):
            log_test_framework("cmcc_devci_email_case3:", "Launch email account pass")
        email.send_email("comcatcmcc2@hotmail.com", "hi", "emailbody")
        sleep(10)        
        if search_text("Please enjoy your day", searchFlag=TEXT_CONTAINS)==False:
            log_test_framework("cmcc_devci_email_case3:", "Send email pass")
            case_flag = True
        elif search_text('has stopped'):
            log_test_framework("cmcc_devci_email_case3:", "Popup has stopped")
            take_screenshot()
            ...
            log_test_framework("cmcc_devci_email_case3:", "Send email fail")
            take_screenshot()
