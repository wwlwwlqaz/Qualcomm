...
#function:
#    move items cross folder in FileExplorer
#precondition:
#    
#step:
#    1.goto FileExplorer; 
...
class test_suit_ui_file_explorer_case07(TestCaseBase):
    tag = 'ui_file_explorer_case07'
    ...
    def test_case_main(self, case_results):
        ...
        #
        # STEP 2: choose items to move
        #
        try:
            (index_list,num1) = random_index_list_in_folder(work_dir,'.jpg')
            log_test_case(self.tag,"num1=%s want to move %s photos"%(str(num1),str(len(index_list)+1)))
            ...
            log_test_case(self.tag, "during MOVE: something wrong, maybe no item in " + cur_path)
            set_cannot_continue()
        ...
        #
        # STEP 4: move items to destination
        #
        if can_continue():
            try:
                ...
                take_screenshot()
                cur_path = get_view_text_by_index(VIEW_TEXT_VIEW,0)
                log_test_case(self.tag, "during MOVE: no 'Paste' in " + cur_path)
                set_cannot_continue()
        ...
        if can_continue():
            ...
            if flag is True:
                ...
                log_test_case(self.tag, "failed move %s"%name_list[i] +'in '+ cur_path)
