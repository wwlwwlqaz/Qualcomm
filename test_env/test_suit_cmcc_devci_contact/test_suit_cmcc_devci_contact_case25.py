...
class test_suit_cmcc_devci_contact_case25(TestCaseBase):
    '''
...
#             phone.permission_allow() 
        if wait_for_fun(lambda:search_text("Contacts", searchFlag=TEXT_CONTAINS), True, 5) or search_text("All contacts") or search_text("ADD NEW ACCOUNT"):
            ...
            sleep(3)
        contact.add_contact_to_phone("exportsim2","1")
        sleep(3)    
        contact.export_contact_to_sim2()
        sleep(3)
        contact.del_one_contact("exportsim2")
        sleep(10)       
        if search_text("exportsim2"):
            log_test_framework(self.name, "Export contact to sim2 successfully")
            case_flag=True
            sleep(2)                   
