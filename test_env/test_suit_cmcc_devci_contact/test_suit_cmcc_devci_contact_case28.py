...
class test_suit_cmcc_devci_contact_case28(TestCaseBase):
    '''
...
#             phone.permission_allow() 
        ...
        for iteration in range(5):
            if wait_for_fun(lambda:search_text("No favorites"), True, 5) or wait_for_fun(lambda:search_text("Frequently contacted"), True, 5) or search_view_by_id("com.android.contacts:id/contact_tile_list_empty"):
                log_test_framework(self.name, "Move contact from favorite successfully")
                case_flag = True
                break;
            else:
                log_test_framework(self.name, "Not found text in iteration %s"%iteration)
        ...
        if search_text("isn't responding"):
            case_flag = False
            if search_text("OK"):
                click_button_by_text("OK")
