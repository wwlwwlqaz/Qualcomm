...
class test_suit_cmcc_devci_contact_case26(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text("Contacts", searchFlag=TEXT_CONTAINS), True, 5) or search_text("All contacts") or search_text("ADD NEW ACCOUNT"):
            ...
            sleep(3)
            contact.add_contact_to_phone("importsim2","1")
            contact.export_contact_to_sim2()
            sleep(3)
            contact.del_one_contact("importsim2")            
            contact.import_contact_from_sim2()
            sleep(3)
            contact.del_one_contact("importsim2")                
            if search_text("importsim2"):
