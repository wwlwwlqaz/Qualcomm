...
class test_suit_cmcc_devci_contact_case34(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text("Contacts", searchFlag=TEXT_CONTAINS), True, 5):
            ...
            sleep(3)
        contact.add_contact_to_coworkers_group()             
        if wait_for_fun(lambda:search_text("No people in this group", searchFlag=TEXT_CONTAINS), True, 10):
            log_test_case("concatct34", "case pass")
            case_flag = True                 
