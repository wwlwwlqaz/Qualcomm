...
class test_suit_cmcc_devci_contact_case35(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text("Contacts", searchFlag=TEXT_CONTAINS), True, 5):
            ...
            sleep(3)
        contact.delete_contact_all_info("y")            
        if wait_for_fun(lambda:search_text("(No name)", searchFlag=TEXT_CONTAINS), False, 5):
            log_test_case("concatct35", "case pass")
            case_flag = True                 
        elif search_text("(No name)", searchFlag=TEXT_CONTAINS):
            log_test_case("concatct35", "save an empty contact")            
