...
class test_suit_cmcc_devci_contact_case32(TestCaseBase):
    '''
...
#             phone.permission_allow()
        ...
        if wait_for_fun(lambda:search_text("y"), True, 5):
            click_textview_by_text("y")
        sleep(3)
        click_imageview_by_id("icon")
        if wait_for_fun(lambda:search_view_by_id("elapsedTime"), True, 15):
            click_button_by_id("floating_end_call_action_button")
        if wait_for_fun(lambda:search_view_by_id("menu_star"), True, 5):
            click_textview_by_id('menu_star')
            sleep(2)
            send_key(KEY_BACK)
        if wait_for_fun(lambda:search_text("Frequently contacted"), True, 5) or wait_for_fun(lambda:is_view_enabled_by_text(VIEW_TEXT_VIEW, "Frequently contacted"), True, 5) or wait_for_fun(lambda:search_text("No favorites"), True, 3):
            log_test_framework(self.name, "Move contact from favorite successfully after dialling")
            case_flag = True
