...
class test_suit_cmcc_devci_camera_case6(TestCaseBase):
    '''
...
#             click_textview_by_text('OK')
        if wait_for_fun(lambda:search_view_by_id('scene_mode_switcher'), True, 10):
            ...
            if search_text('AUTO') == False:
                log_test_framework("cmcc_devci_camera_case6:", "Switch Landscape pass")
                ...
                log_test_framework("cmcc_devci_camera_case6:", "Switch Landscape then camera has stopped")
                take_screenshot()
                click_textview_by_text('OK')
                sleep(2)
        elif search_text('has stopped'):
            log_test_framework("cmcc_devci_camera_case6:", "Camera has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)        
                ...
                sleep(5)
                click_textview_by_text("OK")
                log_test_framework("cmcc_devci_camera_case6:", "ANR")
        else:
            log_test_framework("cmcc_devci_camera_case6:", "Take Landscape photo fail")
        
        click_imageview_by_id('scene_mode_switcher')
        if wait_for_fun(lambda:search_text("AUTO"), True, 5):
            click_textview_by_text('AUTO')
            sleep(3)
        send_key(KEY_BACK)        
        sleep(1)
        send_key(KEY_BACK)
