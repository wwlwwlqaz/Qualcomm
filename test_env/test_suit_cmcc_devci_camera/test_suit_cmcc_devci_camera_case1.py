...
class test_suit_cmcc_devci_camera_case1(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        def execute():  
            ...
            if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
                log_test_framework("cmcc_devci_camera_case1:", "Launch camera pass")
                send_key(KEY_BACK)
                send_key(KEY_BACK)
                send_key(KEY_HOME)
                if wait_for_fun(lambda:search_view_by_desc("Apps list"), True, 3):
                    log_test_framework("cmcc_devci_camera_case1:", "Exit pass")
                    case_flag = True            
            if search_text('stopped'):
                log_test_framework("cmcc_devci_camera_case1:", "Popup has stopped")
                take_screenshot()
                if search_text("OK"):
                ...
                log_test_framework("cmcc_devci_camera_case1:", "Launch camera fail")
