...
def email5(context):
        ...
        if(get_activity_name().endswith("AccountSetupBasics")):
            ...
            if not email.add_email_account(SC.PRIVATE_EMAIL_EMAIL_ACCOUNT_SEQUENCE, SC.PRIVATE_EMAIL_EMAIL_PASSWORD_SEQUENCE):
                qsst_log_case_status(logging_wrapper.STATUS_FAILED, "add email account failed.", logging_wrapper.SEVERITY_HIGH)
                return False
            launcher.launch_from_launcher('email')
        # write an new email
        sleep(2)
        ...
        qsst_log_case_status(logging_wrapper.STATUS_SUCCESS, "send email account success.", logging_wrapper.SEVERITY_HIGH)
        return True
