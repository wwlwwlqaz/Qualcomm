...
class test_suit_scenario1_case5(TestCaseBase):
    '''
    test_suit_scenario1_case3 is a class for scenario1 case.

    @see: L{TestCaseBase <TestCaseBase>}
    '''
    TAG = "test_suit_scenario1_case5"
    '''@var TAG: tag of test_suit_scenario1_case5'''
    def test_case_main(self, case_results):
        ...
        if TestCaseBase.cycle_index%10 == 0:
            goToSleepMode(10*60)
        if fail_num > 0:
            qsst_log_case_status(STATUS_FAILED, "test_suit_scenario1_case5 failed at " + str(TestCaseBase.cycle_index) + " times.", SEVERITY_HIGH)
            return False
        qsst_log_case_status(STATUS_SUCCESS, "test_suit_scenario1_case5 success at " + str(TestCaseBase.cycle_index) + " times.", SEVERITY_HIGH)
