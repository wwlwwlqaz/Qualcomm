...
#function:
#    use the background camera to take a 5M pixels picture
#precondition:
#    there is a mounted sdcard
#steps:
#    open the background camera
#    set the 5M picture size
#    take a picture
############################################


class test_suit_camera_case3(TestCaseBase):
...
#        click_imageview_by_index(index)
        ...
        MAX_COUNT = 15
        count = 0
        while search_text(camera.get_value('5m_pixels'), 1, 0) == False and count < MAX_COUNT:
            count = count + 1
            #click to search the 5M pixel
