...
class test_suit_cmcc_devci_clock_case2(TestCaseBase):
    '''
        ...
        if search_view_by_desc('Alarm'):
            ...
            if search_text('OK'):
                ...
                if search_view_by_id('delete'):
                    log_test_framework("cmcc_devci_clock_case2:", "New clock pass")
                    click_button_by_id('delete')
                    sleep(5)
                    if search_text('Alarm deleted') == True or search_view_by_id('delete') == False:
                        log_test_framework("cmcc_devci_clock_case2:", "Delete clock pass")
                        case_flag = True
        if search_text('has stopped'):
            log_test_framework("cmcc_devci_clock_case2:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)        
        else:
            log_test_framework("cmcc_devci_bt_case1:", "Launch bt fail")        
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)
