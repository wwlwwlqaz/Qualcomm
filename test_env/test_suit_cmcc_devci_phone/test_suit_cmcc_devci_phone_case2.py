...
class test_suit_cmcc_devci_phone_case2(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        sleep(5)
        settings.airplane_mode()#open airplanemode
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_HOME)
        sleep(1)
        browser.browser_baidu()
        if search_text('百度')==False:
            case_flag = True
        ...
        settings.airplane_mode()#close airplanemode
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)
        ...
        if case_flag:
            log_test_framework(self.name, "Website accessed")
            qsst_log_case_status(STATUS_SUCCESS, "" , SEVERITY_HIGH)
        else:
            log_test_framework(self.name, "Could not access the web")
