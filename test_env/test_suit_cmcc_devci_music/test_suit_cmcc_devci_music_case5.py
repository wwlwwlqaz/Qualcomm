...
class test_suit_cmcc_devci_music_case5(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if wait_for_fun(lambda:search_view_by_id("recordButton"), True, 5):
            click_button_by_id('recordButton')
            sleep(60)       
        if wait_for_fun(lambda:search_view_by_id("stopButton"), True, 5):
            click_button_by_id('stopButton')        
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_HOME)
        ...
        if wait_for_fun(lambda:search_text('Artists'), True, 5):
            log_test_framework("cmcc_devci_music_case5:", "Launch music pass") 
        ...
        if wait_for_fun(lambda:search_text("1. recording", searchFlag=TEXT_CONTAINS), True, 5):
            log_test_framework("cmcc_devci_music_case5:", "Delete music pass")            
            case_flag = True                        
        elif search_text('has stopped'):
            log_test_framework("cmcc_devci_music_case5:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("cmcc_devci_music_case5:", "case fail")
            take_screenshot()
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
