...
class test_suit_cmcc_devci_music_case3(TestCaseBase):
    '''
...
#             phone.permission_allow()             
        if wait_for_fun(lambda:search_text('Artists'), True, 5):
            log_test_framework("cmcc_devci_music_case3:", "Launch music pass")            
            click_textview_by_id('search')
            sleep(2)
            entertext_edittext_by_id("search","recordings")
            sleep(5)
            if search_text('Your recordings'):
                log_test_framework("cmcc_devci_music_case3:", "Search music pass")            
                case_flag = True                        
        elif search_text('has stopped'):
            log_test_framework("cmcc_devci_music_case3:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)         
        else:
            log_test_framework("cmcc_devci_music_case3:", "Search music fail")
            take_screenshot()
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
