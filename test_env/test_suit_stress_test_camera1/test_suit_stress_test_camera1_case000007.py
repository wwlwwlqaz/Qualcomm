...
class test_suit_stress_test_camera1_case000007(TestCaseBase):
    '''
    ...
    Step3:Switch to Front Camera;
    Step4:Switch to Video Record for some time;
    Step5:Repeat step2 to step4 for 100 time;
    Verification: 
    ER5:No exception;    
    '''
            ...
            for i in range(iterationNum):
                ...
                if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
                    ...
                    if wait_for_fun(lambda:search_view_by_id("front_back_switcher"), True, 5):
                        ...
                        log_test_framework("step3:", "Switch to front camera pass")
                        sleep(5)
                    if wait_for_fun(lambda:search_view_by_id("shutter_button"), True, 5):                    
                        camera.take_video_camera1(3)
                        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
                            log_test_framework("step4:", "Switch to Video Record use front camera pass")
                            front_flag = True
                            if wait_for_fun(lambda:search_view_by_id('front_back_switcher'), True, 5):
                                click_imageview_by_id('front_back_switcher')
                                sleep(5)
