...
class test_suit_stress_test_camera1_case000020(TestCaseBase):
    '''
    ...
    Step4:Back to camera preview;
    Step5: repeat step2 to step4 for 200 time
    Verification: 
    ...
    def test_case_main(self, case_results):
        ...
        global case_flag , TAG, testresult, success_flag, i, success_time, takepic_flag, del_flag
        case_flag = False
        testresult = []
        success_time = 0
        fail_time = 0
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            for i in range(iterationNum):
                ...
                takepic_flag = False
                del_flag = False
                if wait_for_fun(lambda:search_view_by_id('shutter_button'), False, 5):
                    start_activity('org.codeaurora.snapcam','com.android.camera.CameraLauncher') 
                camera.take_picture()
                if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
                ...
                if wait_for_fun(lambda:search_view_by_id('preview_thumb'), True, 5):
                    ...
                    if wait_for_fun(lambda:search_view_by_id("photopage_bottom_control_delete"), True, 5):
                        ...
                        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 8):
                            log_test_framework("step4:", "Back to camera preview pass")
                    success_flag = takepic_flag and del_flag                            
