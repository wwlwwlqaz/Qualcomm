...
class test_suit_stress_test_camera1_case000008(TestCaseBase):
    '''
    ...
    Step3:Switch to take a picture;
    Step4:Switch to Front Camera;
    Step5:Switch to Video Record for some time;
    Step6:Switch to take a picture;
    Step7:Repeat step1 to step6 for 100 time;
    Verification: 
    ER7:No exception;    
    '''
    ...
    def test_case_main(self, case_results):
        ...
        global case_flag , TAG, testresult, success_flag, i, success_time, rear_video_flag, rear_picture_flag, front_video_flag, front_picture_flag
        case_flag = False
        testresult = []
        success_time = 0
        fail_time = 0
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            for i in range(iterationNum):
                ...
                success_flag = False
                rear_video_flag = False
                rear_picture_flag = False
                front_video_flag = False
                front_picture_flag = False 
                ...
                if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
                    ...
                    rear_video_flag = True                    
                    if wait_for_fun(lambda:search_view_by_id('camera_switcher'), True, 5):
                        click_imageview_by_id("camera_switcher")
                        if wait_for_fun(lambda:search_view_by_desc("Switch to photo"), True, 5):
                            click_imageview_by_desc("Switch to photo")                        
                        camera.take_picture()
                        log_test_framework("step3:", "Switch to take a picture use rear camera pass")
                        rear_picture_flag = True                
                    if wait_for_fun(lambda:search_view_by_id("front_back_switcher"), True, 5):
                        click_imageview_by_id('front_back_switcher')
                        log_test_framework("step4:", "Switch to Front camera pass")
                        sleep(5)
                    if wait_for_fun(lambda:search_view_by_id("shutter_button"), True, 5):                    
                        camera.take_video_camera1(3)
                        if wait_for_fun(lambda:search_view_by_id('front_back_switcher'), True, 5):
                            log_test_framework("step5:", "Switch to Video Record use front camera pass")
                            front_video_flag = True
                            if wait_for_fun(lambda:search_view_by_id('camera_switcher'), True, 5):
                                click_imageview_by_id("camera_switcher")
                                if wait_for_fun(lambda:search_view_by_desc("Switch to photo"), True, 5):
                                    click_imageview_by_desc("Switch to photo")                                
                                camera.take_picture()
                                log_test_framework("step3:", "Switch to take a picture use front camera pass")
                                front_picture_flag = True
                                if wait_for_fun(lambda:search_view_by_id('front_back_switcher'), True, 5):
                                    click_imageview_by_id('front_back_switcher')
                                    sleep(5)
                success_flag = rear_video_flag and rear_picture_flag and front_video_flag and front_picture_flag      
