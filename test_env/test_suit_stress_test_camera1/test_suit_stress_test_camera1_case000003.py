...
class test_suit_stress_test_camera1_case000003(TestCaseBase):
    '''
    ...
    Step2:Switch to Video Record, and record for 2 hour.
    Verification: 
    ER2:The Time icon display correctly and the video is recorded successfully.    
    '''
...
#                     if wait_for_fun(lambda:search_view_by_id('menu'), True, 5):
#                         click_imageview_by_id("menu")                                                                               
#                         if wait_for_fun(lambda:search_text("Video quality"), True, 8):
#                             click_textview_by_text("Video quality")
#                             if wait_for_fun(lambda:search_text("SD 480p"), True, 8):
#                                 click_textview_by_text("SD 480p")        
            camera.take_video_camera1(7200)
            if wait_for_fun(lambda:search_view_by_id("front_back_switcher"), True, 5):
                log_test_framework("step2:", "Switch to rear camera Video Record, and record for 2 hour pass")
                case_flag = True        
