...
class test_suit_stress_test_camera1_case000010(TestCaseBase):
    '''
    ...
    '''
    "Procedure:
    Step1:Launcher Camera and use Front camera;
    Step2:Take picture for 200 time
    Verification: 
    ER2:No exception;    
    '''
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):      
            if wait_for_fun(lambda:search_view_by_id('front_back_switcher'), True, 5):
                ...
                for i in range(iterationNum):
                    ...
                    camera.take_picture()
                    if wait_for_fun(lambda:search_view_by_id("shutter_button"), True, 5):
                        log_test_framework("step2:", "Take picture use front camera pass")
                        success_flag = True               
