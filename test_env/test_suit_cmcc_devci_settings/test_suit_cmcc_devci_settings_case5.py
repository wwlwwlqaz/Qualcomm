...
class test_suit_cmcc_devci_settings_case5(TestCaseBase):
    '''
...
#             phone.permission_allow()                  
        if wait_for_fun(lambda:search_text('WLAN'), True, 5) or wait_for_fun(lambda:search_text('Bluetooth'), True, 5):
            log_test_framework("cmcc_devci_settings_case5:", "Launch settings pass")
            settings.change_font_size_to_default()
            if wait_for_fun(lambda:search_text("Default"), True, 3):
                log_test_framework("cmcc_devci_settings_case5:", "case pass")
                case_flag = True
        elif search_text("stopped", searchFlag=TEXT_CONTAINS):
            log_test_framework("cmcc_devci_settings_case5:", "Popup has stopped")
            take_screenshot()
            if search_text("OK"):
            ...
            log_test_framework("cmcc_devci_settings_case5:", "case fail")        
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)
