...
class test_suit_cmcc_devci_gallery_case9(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text('Timeline'), True, 5):
            log_test_framework("cmcc_devci_gallery_case2:", "Launch gallery pass")      
        elif wait_for_fun(lambda:search_text('Albums'), True, 5):
            log_test_framework("cmcc_devci_gallery_case2:", "Launch gallery pass")   
        gallery.edit_sharpness()           
        if search_text('stopped')==False:            
            log_test_framework("cmcc_devci_gallery_case9:", "case pass")
            case_flag = True       
        elif search_text('has stopped'):
            log_test_framework("cmcc_devci_gallery_case9:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("cmcc_devci_gallery_case9:", "case fail")
