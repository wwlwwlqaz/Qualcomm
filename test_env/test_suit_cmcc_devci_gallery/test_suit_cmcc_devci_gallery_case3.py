...
class test_suit_cmcc_devci_gallery_case3(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text('Timeline'), True, 5):
            log_test_framework("cmcc_devci_gallery_case3:", "Launch gallery pass")      
        elif wait_for_fun(lambda:search_text('Albums'), True, 5):
            log_test_framework("cmcc_devci_gallery_case3:", "Launch gallery pass")   
        click_textview_by_id('action_camera')
        if wait_for_fun(lambda:search_view_by_id('permission_allow_button'), True, 10):
            phone.permission_allow()
        if wait_for_fun(lambda:search_text('Yes'), True, 3):            
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):            
            log_test_framework("cmcc_devci_gallery_case3:", "Go into camera pass")
            case_flag = True       
        elif search_text('has stopped'):
            log_test_framework("cmcc_devci_gallery_case3:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("cmcc_devci_gallery_case3:", "Go into camera fail")
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)
