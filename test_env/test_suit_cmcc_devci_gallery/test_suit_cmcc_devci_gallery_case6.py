...
class test_suit_cmcc_devci_gallery_case6(TestCaseBase):
    '''
        ...
        if wait_for_fun(lambda:search_view_by_id("photopage_bottom_control_edit"), True, 5):
            ...
            if wait_for_fun(lambda:search_text("Details"), True, 5):
                ...
                if wait_for_fun(lambda:search_text("QCOM-AA", searchFlag=TEXT_CONTAINS), True, 5):
                    log_test_framework("cmcc_devci_gallery_case6:", "maker information display QCOM-AA")
                    case_flag = True       
        elif search_text('has stopped'):
            log_test_framework("cmcc_devci_gallery_case6:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
                ...
                sleep(5)
                click_textview_by_text("OK")
                log_test_framework("cmcc_devci_gallery_case6:", "ANR")
        else:
            log_test_framework("cmcc_devci_gallery_case6:", "case fail")
