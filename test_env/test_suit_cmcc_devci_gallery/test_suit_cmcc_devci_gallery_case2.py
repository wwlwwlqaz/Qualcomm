...
class test_suit_cmcc_devci_gallery_case2(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if search_text('No photos found')==False:
            log_test_framework("cmcc_devci_gallery_case2:", "Check gallery pass")
            case_flag = True       
        if search_text('has stopped'):
            log_test_framework("cmcc_devci_gallery_case2:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("cmcc_devci_gallery_case2:", "Check gallery fail")
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)
