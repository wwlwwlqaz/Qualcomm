...
class test_suit_cmcc_devci_message_case33(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            sleep(1)
        mms.set_text_message_limit('10')
        mms.send_text_message_limit()   
        if search_text('one')==False:
            log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Set message limit successfully")
            case_flag=True                  
