...
class test_suit_cmcc_devci_message_case24(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            if wait_for_fun(lambda:search_text('No conversations'), True, 5) or search_text("Notifications", searchFlag=TEXT_CONTAINS):
                log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "delete all threads succussfully")
                case_flag=True   
