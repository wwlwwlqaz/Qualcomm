...
class test_suit_cmcc_devci_message_case45(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        global case_flag , TAG,case_flag_lock,case_flag_unlock, i
        case_flag = False
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 3) or wait_for_fun(lambda:search_view_by_id("create"), True, 3):
            ...
            mms.send_sms("18721465135", "content")
            if wait_for_fun(lambda:search_text("China Mobile", searchFlag=TEXT_CONTAINS), True, 60) or wait_for_fun(lambda:search_text("CARD 1", searchFlag=TEXT_CONTAINS), True, 60):
                log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Check SUB-1 details of SMS  successfully")
                case_flag=True
            else:
                log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Check SUB-1 details of SMS  failed")
                take_screenshot()                     
