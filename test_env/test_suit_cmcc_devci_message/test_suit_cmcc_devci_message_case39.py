...
class test_suit_cmcc_devci_message_case39(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            sleep(1)
        mms.discard_sms("13916371096", "discard")        
        if search_text("discard", searchFlag=TEXT_CONTAINS)==False:
            log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Discard SMS successfully")
            case_flag=True                  
