...
class test_suit_cmcc_devci_message_case35(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        global case_flag , TAG,case_flag_sim1,case_flag_sim2, i
        case_flag = False
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            sleep(1)
        mms.preferred_storage("SIM card")
        mms.send_sms("10086", "10086")
        send_key(KEY_BACK)
        sleep(2)
        send_key(KEY_BACK)
        sleep(2)
        mms.manage_SIM1_card_message()
        if wait_for_fun(lambda:search_text("10086", searchFlag=TEXT_CONTAINS), True, 120):
            log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Set SIM card storage successfully")
            case_flag=True                  
