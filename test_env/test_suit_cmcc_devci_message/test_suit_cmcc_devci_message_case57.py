...
class test_suit_cmcc_devci_message_case57(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            mms.add_recipient('13936583079')
            sleep(3)
            mms.add_picture()
            click_imageview_by_id('send_button_mms')
            if wait_for_fun(lambda:search_text("Now"), True, 120):
                click_textview_by_text("Now", clickType=LONG_CLICK)
                sleep(2)
                click_textview_by_id("forward")
                sleep(3)
                entertext_edittext_by_id("recipients_editor", "13836583079")
                sleep(3)
                send_key(KEY_ENTER)
                sleep(2)
                click_button_by_id("send_button_mms")
                if wait_for_fun(lambda:search_text("Now"), True, 120):
