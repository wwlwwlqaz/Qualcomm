...
class test_suit_cmcc_devci_message_case28(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            sleep(1)
        mms.send_sms("13916371096", "search")
        send_key(KEY_BACK)
        sleep(5)
        send_key(KEY_BACK)
        sleep(5)
        mms.search_sms('search')
        sleep(3)
        if wait_for_fun(lambda:search_text('139 1637 1096'), True, 10):
            log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Search sms successfully")
            case_flag=True                  
