...
class test_suit_cmcc_devci_message_case30(TestCaseBase):
    '''
...
#             phone.permission_allow()
        ...
        if wait_for_fun(lambda:search_text("Change to folder mode"), True, 5):
            click_textview_by_text('Change to folder mode')
        if wait_for_fun(lambda:search_text('Inbox'), True, 5):
            log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Change folder mode successfully")
            case_flag=True                  
        ...
        click_imageview_by_desc('More options')
        if wait_for_fun(lambda:search_text("Change to conversation mode"), True, 5):
            click_textview_by_text('Change to conversation mode')
            sleep(3)                
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
