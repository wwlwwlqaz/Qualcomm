...
class test_suit_cmcc_devci_message_case31(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            sleep(1)
        mms.sms_signature("scj")
        mms.send_sms("13916371096", "signature")
        if wait_for_fun(lambda:search_text("scj",searchFlag=TEXT_CONTAINS), True, 120):
        ...
        if wait_for_fun(lambda:search_text("SMS signature enabled"), True, 5):
            click_textview_by_text('SMS signature enabled')
            sleep(3)        
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
