...
class test_suit_cmcc_devci_message_case38(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_view_by_id("action_compose_new"), True, 5) or wait_for_fun(lambda:search_view_by_id("create"), True, 5):
            ...
            sleep(1)
        mms.send_add_subject("13916371096")
        if wait_for_fun(lambda:search_text("subject"), True, 120):
            log_test_case(self.case_config_map[fs_wrapper.CASE_NAME_ATTR], "Add subject successfully")
            case_flag=True                  
