...
class test_suit_mms_case1(TestCaseBase):
    '''
    test_suit_mms_case1 is a class for mms case.

    @see: L{TestCaseBase <TestCaseBase>}
    '''
    TAG = "test_suit_mms_case1"
    '''@var TAG: tag of test_suit_mms_case1'''
    def test_case_main(self, case_results):
        ...
        if not wait_for_fun(func, True, 30):
            mms.click_home_icon()
            set_cannot_continue()
            log_test_case(self.TAG, "Sent SMS on slot1 failed.")
        mms.click_home_icon()
        ...
        if not search_text(mms.get_value("received"), searchFlag=TEXT_CONTAINS):
            mms.click_home_icon()
            set_cannot_continue()
            log_test_case(self.TAG, "Received SMS on slot1 failed.")
        mms.click_home_icon()
